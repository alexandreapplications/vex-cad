      ...
      <Grid container>
        ...
        </Grid>
        {
          props.record.emails.map((item, idx) => (
            <Email record={item} key={idx} idx={idx} onChange={props.onChange}></Email>
          ))
        }
        {
          props.record.telefones.map((item, idx) => (
            <Telefone record={item} key={idx} idx={idx} onChange={props.onChange}></Telefone>
          ))
        }
        {
          props.record.enderecos.map((item, idx) => (
            <Endereco record={item} key={idx} idx={idx} onChange={props.onChange}></Endereco>
          ))
        }
        <Grid item xs={12} sm={6} className={classes.paper}>
          <FormControl fullWidth={true}>
